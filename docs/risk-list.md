...
| `R-03` | File and directory resolution can expose files outside the requested repository. | Technical / security | Low | High | E2 canonicalizes the document root, excludes discovered symbolic links and hidden entries, and serves only known document identifiers. Traversal and symlink behavior are tested. C2 verifies in a browser that a hidden Markdown file receives a Lens guidance page instead of its content. Large-repository discovery limits remain a residual risk. |
