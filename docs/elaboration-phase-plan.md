...
Reduce the renderer privacy, content-safety, target-scoping, and product-scope
risks enough to commit to a small first release plan and a viable architecture.
...
|---|---|---|
| `E1` | Prove a safe file-target vertical slice and decide the renderer boundary. | Executable spike, target-scoping tests, renderer privacy decision, and updated risks. |
...
- PlantUML renderer behavior, privacy, and failure handling are decided.
