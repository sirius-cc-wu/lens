...
description: "Product requirements, user stories, example mapping, and acceptance criteria for rendering qualifying repository source files directly within the Lens browser viewer."
...
Under the previous behavior defined in ADR-021, selecting a qualifying relative source link forced an external application handoff by opening Visual Studio Code via `vscode://file/...`. While this bypassed the need to serve source files over HTTP at the time, it introduced significant user friction:
...
- **Platform Incompatibilities:** On machines where VS Code is not installed, in browser-based containers, or over remote headless sessions, selecting a source link results in browser error alerts or dead-ends (`The address wasn't understood` or `No application is registered to handle this link`).
...
Enable developers and technical writers to inspect referenced repository source files directly inside the Lens browser interface. Source links navigate seamlessly to a clean, line-numbered, in-browser code view that supports deep linking to specific lines, visual line highlighting, automatic refresh upon file changes, and an optional, non-intrusive "Open in VS Code" header link for when editing is genuinely required.
...
### Story 3: Optional Editor Handoff
**As a** developer who discovers a bug while reading code in Lens,  
**I want an** explicit "Open in VS Code" button on the rendered source code page,  
**So that** I can seamlessly transition to editing when I choose to, without being forced into it during casual reading.

---

## 3. Collaborative Requirements Discovery (Example Mapping)

To solidify acceptance rules and boundary handling, the team conducted an Example Mapping workshop across business rules, edge cases, and failure modes.

```
                           ┌────────────────────────────────────────────────────────┐
                           │                       User Story                       │
                           │     Render qualifying source code directly in Lens     │
                           └──────────────────────────┬─────────────────────────────┘
                                                      │
         ┌───────────────────┬────────────────────────┼────────────────────────┬───────────────────┐
         ▼                   ▼                        ▼                        ▼                   ▼
    ...
    │ Route &  │       │ Secure    │            │ Line      │            │ Content   │       │ Optional  │
    │ Rewrite  │       │ On-Demand │            │ Targeting │            │ Guards    │       │ Editor    │
    └────┬─────┘       └─────┬─────┘            └─────┬─────┘            └─────┬─────┘       └─────┬─────┘
         │                   │                        │                        │                   │
         ├─────────┐         ├─────────┐              ├─────────┐              ├─────────┐         │
         ▼         ▼         ▼         ▼              ▼         ▼              ▼         ▼         ▼
     [Ex 1.1]  [Ex 1.2]  [Ex 2.1]  [Ex 2.2]       [Ex 3.1]  [Ex 3.2]       [Ex 4.1]  [Ex 4.2]  [Ex 5.1]
     Relative  Doc route Validated Root-crossing  #L42      No fragment    Binary    >2 MB     Header
     file link takes     regular   rejected       scrolls   renders whole  rejected  rejected  link opens
     -> /source priority file      404            & glows   file           notice    notice    in VS Code
```

### Business Rules & Concrete Examples

#### Rule 1: Link Resolution & Rewrite Precedence
*Relative links in Markdown documents targeting qualifying source files must rewrite to `/source/{path}` with authored fragments, while discovered documents retain `/documents/`.*
- **Example 1.1 (Standard source link):** A document contains `[Main](src/main.rs#L10)`. Lens rewrites the destination to `/source/src/main.rs?token=...#L10` and omits any `(opens in VS Code)` suffix.
...
- **Example 1.3 (External / absolute URLs):** `https://github.com` or `/absolute/path` retain their exact authored destinations without `/source/` rewriting.
...
#### Rule 5: Non-Disruptive Optional Editor Access
*The rendered source page must offer an explicit, non-blocking action to open the file in VS Code.*
- **Example 5.1 (Header action):** At the top of `/source/src/main.rs`, an action button labeled `Open in VS Code` points to `vscode://file/{canonical_absolute_path}`. Selecting it lets the operating system open VS Code without having disrupted the initial browser reading session.

#### Rule 6: Automatic Refresh Support
...
- **Example 6.1 (Live refresh):** The user is viewing `/source/src/main.rs`. In their editor, they edit `main.rs` and save. Lens's background revision poller detects the file modification and refreshes the browser page seamlessly.
