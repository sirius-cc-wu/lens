...
| Lens client | The short-lived part of an ordinary `lens` invocation that submits one open request, prints the ready loopback URL, reports immediate failures, and returns control to the terminal. |
...
| Browser session | The local HTTP session Lens starts for a resolved target; the user opens its printed URL in a browser. |
