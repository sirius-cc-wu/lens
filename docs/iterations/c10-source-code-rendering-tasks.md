...
title: "Iteration C10: In-Browser Source Code Rendering Implementation"
description: "Structured work packages, test specifications, and verification checklists for downstream Workers implementing in-browser source rendering."
id: "C10"
status: "ready"
tags: [iteration, tasks, planning, worker-handoff]
---

# Iteration C10: In-Browser Source Code Rendering Implementation Tasks
...
### Slice 1: Source Content Authorization & Inspection (`src/source_link.rs`)

**Objective:** Extend `SourceLinkResolver` to read and authorize source file contents safely on-demand with size caps and UTF-8 verification.
...
- [ ] **Task 1.3:** Write 3A unit tests following `<condition_or_action>_then_<observable_result>` naming:
...
### Slice 2: Markdown Link Rewriting (`src/markdown.rs`)

**Objective:** Transition relative link rewriting from `vscode://file/...` to `/source/...` destinations.
...
- [ ] **Task 2.1:** Update `resolve_link` in `src/markdown.rs`:
  ...
  - Set `opens_in_vscode: false` to remove the inline `(opens in VS Code)` indicator span.
...
- [ ] **Task 3.1:** Add `source_page` in `src/viewer/page.rs`:
  - Render breadcrumb navigation and file metadata.
  - Format code in semantic table with `<tr id="L{n}">` and `<td class="line-num"><a href="#L{n}">{n}</a></td>`.
  - Provide optional "Open in VS Code" header link pointing to `vscode_url`.
...
- [ ] A non-disruptive "Open in VS Code" button is available in the header.
